...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "SELECT m.title, m.year, COUNT(r.rating) AS high_ratings_count FROM movies AS m JOIN ratings AS r ON m.id = r.movie_id JOIN users AS u ON r.user_id = u.id WHERE m.genres LIKE '%%Drama%%' AND m.year > 2005 AND u.gender = 'female' AND r.rating >= 4.5 GROUP BY m.id, m.title, m.year ORDER BY m.year, m.title;"
echo " "
