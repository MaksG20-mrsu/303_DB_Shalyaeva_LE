...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "SELECT u1.name AS user1, u2.name AS user2, m.title FROM ratings AS r1 JOIN ratings AS r2 ON r1.movie_id = r2.movie_id JOIN users AS u1 ON r1.user_id = u1.id JOIN users AS u2 ON r2.user_id = u2.id JOIN movies AS m ON r1.movie_id = m.id WHERE r1.user_id < r2.user_id GROUP BY u1.name, u2.name, m.title ORDER BY m.title, u1.name, u2.name LIMIT 100"
echo " "
...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "SELECT DISTINCT m.title, u.name, r.rating, date(r.timestamp, 'unixepoch') AS review_date FROM ratings AS r JOIN users AS u ON r.user_id = u.id JOIN movies AS m ON r.movie_id = m.id ORDER BY r.timestamp ASC LIMIT 10"
echo " "
...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "WITH avg_ratings AS (SELECT m.id, m.title, m.year, AVG(r.rating) AS avg_rating FROM movies AS m JOIN ratings AS r ON m.id = r.movie_id GROUP BY m.id, m.title, m.year), min_max AS (SELECT MIN(avg_rating) AS min_rating, MAX(avg_rating) AS max_rating FROM avg_ratings) SELECT ar.year, ar.title, ar.avg_rating, CASE WHEN ar.avg_rating = mm.max_rating THEN 'Да' ELSE 'Нет' END AS Рекомендуем FROM avg_ratings ar CROSS JOIN min_max AS mm WHERE ar.avg_rating = mm.min_rating OR ar.avg_rating = mm.max_rating ORDER BY ar.year, ar.title"
echo " "
...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "SELECT COUNT(*) AS количество_оценок, ROUND(AVG(r.rating), 2) AS средняя_оценка FROM ratings AS r JOIN users AS u ON r.user_id = u.id WHERE u.gender = 'male' AND strftime('%Y', date(r.timestamp, 'unixepoch')) BETWEEN '2011' AND '2014'"
echo " "
...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "SELECT m.year, m.title, ROUND(AVG(r.rating), 2) AS средняя_оценка, COUNT(DISTINCT r.user_id) AS количество_пользователей FROM movies AS m JOIN ratings AS r ON m.id = r.movie_id GROUP BY m.year, m.title ORDER BY m.year, m.title LIMIT 20"
echo " "
...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "WITH RECURSIVE birthday_years AS (SELECT 1990 AS year UNION ALL SELECT year + 1 FROM birthday_years WHERE year < 2024) SELECT year AS год, CASE CAST(strftime('%w', year || '-11-07') AS INTEGER) WHEN 0 THEN 'Воскресенье' WHEN 1 THEN 'Понедельник' WHEN 2 THEN 'Вторник' WHEN 3 THEN 'Среда' WHEN 4 THEN 'Четверг' WHEN 5 THEN 'Пятница' WHEN 6 THEN 'Суббота' END AS день_недели FROM birthday_years"
echo " "